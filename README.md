...
- **Prefix convention:** every resource a student creates starts with their user ID (`student-user1-…`, `Items-user1`, `lab4-user1`, `StudentLambdaRole-user1`). This is a **naming convention** that avoids collisions in the shared account — it is *not* IAM-enforced (students are admins), so the labs and `bootstrap.sh` use deterministic `userN`-derived names to stay out of each other's way.
